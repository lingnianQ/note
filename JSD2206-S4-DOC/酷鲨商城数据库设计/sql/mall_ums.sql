...
CREATE TABLE `ums_change_password_log` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户修改密码日志表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户基本（常用）信息表';
...
CREATE TABLE `ums_user_detail` (
  ...
  `gmt_modified` datetime DEFAULT NULL COMMENT '数据最后修改时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户详细（不常用）信息表';
